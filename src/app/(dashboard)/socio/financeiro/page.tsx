...
export default async function SocioFinancialPage() { await requireRole("socio"); const [financial, studentsResult, report] = await Promise.all([listPlansAndSubscriptions(), listStudents(), getFinancialReport()]); const students = "data" in studentsResult ? studentsResult.data ?? [] : []; return <div className="dashboard-page"><p className="eyebrow">Gestão financeira</p><h1 className="dashboard-title">Planos, contratos e cobranças</h1><p className="dashboard-subtitle">As cobranças são geradas por ciclo, com valor preservado no contrato e sem duplicação.</p><section className="dashboard-section"><Card elevated><FinancialReport data={report}/><FinancialEntryForm />{"error" in financial?<p className="form-error" role="alert">{financial.error}</p>:<PlanManagement plans={financial.data.plans as never} versions={financial.data.versions as never} subscriptions={financial.data.subscriptions as never} charges={financial.data.charges as never} runs={financial.data.runs as never} students={students as never}/>}</Card></section></div>; }

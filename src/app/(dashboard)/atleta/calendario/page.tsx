...
export default async function AthleteCalendarPage({
  ...
  return <div className="dashboard-page"><p className="eyebrow">Painel do atleta</p><h1 className="dashboard-title">{heading}</h1><p className="dashboard-subtitle">{subtitle}</p><section className="dashboard-section"><Card elevated>{error ? <p className="form-error" role="alert">Não foi possível carregar seus treinos.</p> : data?.length ? <ul className="dashboard-list">{data.map((item) => { const training = Array.isArray(item.treinos) ? item.treinos[0] : item.treinos; const date = item.agendado_para ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "full", timeStyle: "short", timeZone: item.timezone ?? "UTC" }).format(new Date(item.agendado_para)) : ""; return <li key={item.id}><Link className="dashboard-list-title" href={`/atleta/treinos/${item.id}`}>{training?.titulo ?? "Treino"}</Link><span className="dashboard-list-when">{date}</span><span className="dashboard-list-detail">{item.observacao_treinador ?? training?.descricao ?? "Sem observação"}</span><TrainingExecutionForm assignmentId={item.id} status={item.status} /></li>; })}</ul> : <p className="dashboard-empty-state">Nenhum treino agendado.</p>}</Card></section></div>;
}

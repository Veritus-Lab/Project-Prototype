...
export default async function StudentsPage() {
  ...
  return <div className="dashboard-page">
    ...
    <section className="dashboard-section"><Card elevated><h2>Alunos cadastrados</h2>{"error" in result ? <p className="form-error" role="alert">{result.error}</p> : students.length ? <ul className="dashboard-list">{students.map((student) => <li key={student.id}><span className="dashboard-list-title">{student.name}</span><span className="dashboard-list-detail">{student.email ?? student.phone ?? "Sem contato informado"}</span><EnrollmentControls studentId={student.id} enrollment={student.enrollment} /></li>)}</ul> : <p className="dashboard-empty-state">Nenhum aluno cadastrado.</p>}</Card></section>
  </div>;
}

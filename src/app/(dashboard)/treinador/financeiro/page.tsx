...
export default async function FinancialPage() {
  ...
  return <div className="dashboard-page"><p className="eyebrow">Gestão financeira</p><h1 className="dashboard-title">Financeiro</h1><p className="dashboard-subtitle">Assinaturas, vencimentos e pagamentos registrados manualmente.</p><section className="dashboard-section"><Card elevated><h2>Nova assinatura</h2>{athletes.length ? <SubscriptionForm athletes={athletes} /> : <p className="dashboard-empty-state">Vincule um aluno antes de criar uma assinatura.</p>}</Card></section><section className="dashboard-section"><Card elevated><h2>Assinaturas</h2>{subscriptionsError ? <p className="form-error">Não foi possível carregar as assinaturas.</p> : subscriptions?.length ? <ul className="dashboard-list financial-subscription-list">{subscriptions.map((subscription) => <li key={subscription.id}><span className="dashboard-list-title">{athleteNames.get(subscription.atleta_id) ?? "Aluno"}</span><span className="dashboard-list-detail">{amount.format(subscription.valor_centavos / 100)} · {subscription.periodicidade} · dia {subscription.dia_vencimento}</span><SubscriptionStatusForm subscriptionId={subscription.id} status={subscription.status} /></li>)}</ul> : <p className="dashboard-empty-state">Nenhuma assinatura cadastrada.</p>}</Card></section><section className="dashboard-section"><Card elevated><h2>Consentimento de WhatsApp</h2><p className="field-hint">A plataforma não envia mensagens nesta etapa.</p>{athletes.length ? <CommunicationPreferenceForm athletes={athletes} /> : null}</Card></section><section className="dashboard-section"><Card elevated><h2>Próximas cobranças</h2>{chargesError ? <p className="form-error">Não foi possível carregar as cobranças.</p> : charges?.length ? <ul className="dashboard-list">{charges.map((charge) => { const status = financialStatus({ dueDate: charge.vencimento_em, status: charge.status }); return <li key={charge.id}><span className="dashboard-list-title">{athleteNames.get(charge.atleta_id) ?? "Aluno"}</span><span className="dashboard-list-when">{status}</span><span className="dashboard-list-detail">{amount.format(charge.valor_centavos / 100)} · vence em {new Intl.DateTimeFormat("pt-BR").format(new Date(`${charge.vencimento_em}T00:00:00`))}</span>{charge.status === "aberta" ? <><ChargePaidForm chargeId={charge.id} /><QueueReminderForm chargeId={charge.id} /></> : null}</li>; })}</ul> : <p className="dashboard-empty-state">Nenhuma cobrança cadastrada.</p>}</Card></section></div>;
}

import { ChargePaidForm, CommunicationPreferenceForm, QueueReminderForm, SubscriptionForm } from "@/components/dashboard/financial-forms";
...
export default async function FinancialPage() {
  ...
  return <div className="dashboard-page"><p className="eyebrow">Painel do treinador</p><h1 className="dashboard-title">Financeiro</h1><p className="dashboard-subtitle">Assinaturas, vencimentos e pagamentos registrados manualmente.</p><section className="dashboard-section"><Card elevated><h2>Nova assinatura</h2>{athletes.length ? <SubscriptionForm athletes={athletes} /> : <p className="dashboard-empty-state">Vincule um atleta antes de criar uma assinatura.</p>}</Card></section><section className="dashboard-section"><Card elevated><h2>Consentimento de WhatsApp</h2><p className="field-hint">A plataforma não envia mensagens nesta etapa.</p>{athletes.length ? <CommunicationPreferenceForm athletes={athletes} /> : null}</Card></section><section className="dashboard-section"><Card elevated><h2>Próximas cobranças</h2>{error ? <p className="form-error">Não foi possível carregar as cobranças.</p> : charges?.length ? <ul className="dashboard-list">{charges.map((charge) => { const status = financialStatus({ dueDate: charge.vencimento_em, status: charge.status }); return <li key={charge.id}><span className="dashboard-list-title">{athleteNames.get(charge.atleta_id) ?? "Atleta"}</span><span className="dashboard-list-when">{status}</span><span className="dashboard-list-detail">{amount.format(charge.valor_centavos / 100)} · vence em {new Intl.DateTimeFormat("pt-BR").format(new Date(`${charge.vencimento_em}T00:00:00`))}</span>{charge.status === "aberta" ? <><ChargePaidForm chargeId={charge.id} /><QueueReminderForm chargeId={charge.id} /></> : null}</li>; })}</ul> : <p className="dashboard-empty-state">Nenhuma cobrança cadastrada.</p>}</Card></section></div>;
}

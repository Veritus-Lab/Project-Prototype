...
export default async function TrainerCalendarPage() {
  ...
  return <div className="dashboard-page"><p className="eyebrow">Painel do treinador</p><h1 className="dashboard-title">Calendário</h1><p className="dashboard-subtitle">Planeje os próximos treinos da assessoria.</p><section className="dashboard-section"><Card elevated><h2>Novo agendamento</h2>{"error" in athleteResult || "error" in trainingResult ? <p className="form-error" role="alert">Não foi possível carregar os dados de agendamento.</p> : <TrainingScheduleForm athletes={athletes} trainings={trainings} />}</Card></section></div>;
}

...
| Proteção de último sócio | PASSOU no banco remoto | Trigger `prevent_last_socio_removal` está presente e serializa alterações concorrentes por assessoria; pgTAP adiciona os cenários de remoção permitida e bloqueada para o CI. |
| Confirmação do e-mail de equipe | PASSOU em revisão | O RPC exige `email_confirmed_at` antes de criar perfil ou papel; o pgTAP cobre o bloqueio. |

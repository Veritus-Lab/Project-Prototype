...
| Estados separados | Máquinas de matrícula, assinatura, cobrança, checkout, pagamento/reversão, despesa, evento, mensagem e lead | Passou |
...
| Professor limitado | Decisões 7, endpoint agregado e RLS | Passou: somente `em_dia`, `pendente`, `nao_configurado`, `indisponivel` |
...
| Pagamento confiável | Checkout próprio; redirect não paga; evento/baixa transacional | 16, 19–21 | Duplicação, ordem, falha parcial e acesso alheio (QA-E/F/G) |
| Professor vê quatro estados sem detalhe | Contrato retorna somente enum; professor sem leitura financeira | 07, 20, 25 | `nao_configurado`/erro não viram `em_dia`; payload não contém detalhes (QA-C/F/G) |
...
| Falhas recuperáveis | Eventos duráveis, retry limitado, dead-letter, reconciliação e replay auditado | 20, 23, 29, 31 | Evento perdido, timeout, worker concorrente e replay (QA-F/H) |
...
| Indicadores financeiros honestos | Cobrança, pagamento, reversão, despesa e movimentos separados | 16–18, 24 | Realizado/previsto, taxas e falha não exibida como zero (QA-E/G) |
...
- Revisão cruzada confirmou autenticação de cron/webhooks, ownership de aluno/FLERNK, chaves de idempotência, resposta rápida, fila, dead-letter e reconciliação.
- Revisão da matriz de falhas cobriu duplicação, desordem, evento perdido, falha depois de chamada externa, concorrência, opt-out e quitação após enqueue.

...
5. O scheduler primário é o **Vercel Cron no plano Hobby**, em UTC, com uma execução diária e tolerância de horário. Um endpoint interno autenticado executa, em lotes limitados, geração de cobranças, lembretes e reconciliação; webhooks continuam processando fatos externos quando chegam. O Supabase Postgres guarda a fonte de verdade, locks, filas e eventos duráveis. Não será contratado scheduler pago no primeiro momento, e qualquer alternativa gratuita futura exige ADR e mantém um único scheduler ativo.

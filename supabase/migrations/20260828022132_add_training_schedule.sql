...
alter table public.treinos_atletas
  ...
  add column if not exists observacao_treinador text;

alter table public.treinos_atletas
  drop constraint if exists treinos_atletas_treino_atleta_key,
...
create unique index if not exists treinos_atletas_treino_atleta_sem_agendamento_key
  on public.treinos_atletas (treino_id, atleta_id)
  where agendado_para is null;

create index if not exists treinos_atletas_assessoria_agendado_para_idx

...
const trainingSeeds = [
  { titulo: "Intervalado 6x400m", descricao: "Aquecimento, seis tiros de 400 m e desaquecimento.", offset: 0 },
  { titulo: "Rodagem regenerativa", descricao: "Ritmo confortável, priorizando recuperação.", offset: 2 },
  { titulo: "Longão progressivo", descricao: "Construir o ritmo gradualmente até o final.", offset: 4 },
  { titulo: "Tempo run", descricao: "Bloco contínuo em ritmo sustentável.", offset: -2 },
];
const { data: trainings, error: trainingError } = await supabase.from("treinos").insert(trainingSeeds.map((training) => ({
  ...
  origem: "manual", estrutura: { blocos: [] },
}))).select("id, titulo");
...
const assignments = athleteIds.map((athleteId, index) => ({
  ...
  atleta_id: athleteId,
  status: index === 3 ? "concluido" : index === 4 ? "em_andamento" : "atribuido",
  agendado_para: at(trainingSeeds[index % trainingSeeds.length].offset, 7 + index),
  timezone: "America/Sao_Paulo",
  iniciado_em: index >= 3 ? at(-2, 7) : null,
  concluido_em: index === 3 ? at(-1, 8) : null,
